@extends('layouts.app')

@section('title', 'IT Solutions | Webvire - Enterprise Cloud, Security & ERP Software')
@section('meta_description', 'Enterprise technology solutions by Webvire: Cloud Architecture, Cybersecurity, ERP & CRM Systems, AI Machine Learning, DevOps, and 24/7 Managed IT Support.')
@section('meta_keywords', 'Enterprise IT Solutions, Cloud Architecture, Cybersecurity, ERP CRM Systems, AI Machine Learning, DevOps, Managed IT')

@section('content')
  <!-- Subpage Hero -->
  <section class="subpage-hero">
    <div class="container">
      <div class="pill-badge">ENTERPRISE IT SOLUTIONS</div>
      <h1>Smart Technology Solutions for <span class="gradient-text">Better Growth</span></h1>
      <p>Custom software engineering, cloud architecture, cybersecurity, and AI integration engineered to modernize enterprise operations.</p>
    </div>
  </section>

  <!-- Security Certifications Strip Bar -->
  <div class="feature-strip">
    <div class="container">
      <div class="features-grid">
        <div class="feature-card">
          <div class="feature-icon-box icon-blue"><i class="fa-solid fa-user-shield"></i></div>
          <h3>SOC2 Type II Hardening</h3>
          <p>Strict data privacy & security standards.</p>
        </div>
        <div class="feature-card">
          <div class="feature-icon-box icon-rose"><i class="fa-solid fa-lock"></i></div>
          <h3>AES-256 Encryption</h3>
          <p>Data encrypted in transit & at rest.</p>
        </div>
        <div class="feature-card">
          <div class="feature-icon-box icon-purple"><i class="fa-solid fa-server"></i></div>
          <h3>Kubernetes Auto-Scale</h3>
          <p>Zero downtime under peak traffic surges.</p>
        </div>
        <div class="feature-card">
          <div class="feature-icon-box icon-cyan"><i class="fa-solid fa-clock-rotate-left"></i></div>
          <h3>Automated Hourly Backups</h3>
          <p>Disaster recovery & instant snapshot restore.</p>
        </div>
      </div>
    </div>
  </div>

  <!-- 6 Enterprise IT Solutions Grid -->
  <section class="section section-alt">
    <div class="container">
      <div class="section-head">
        <div class="pill-badge">CORE IT SOLUTIONS</div>
        <h2>Enterprise Technology Services</h2>
        <p>Robust, secure, and scalable IT solutions tailored to simplify complex business workflows.</p>
      </div>

      <div class="services-grid-ref">
        <div class="service-card-ref">
          <div class="service-icon-circle icon-blue"><i class="fa-solid fa-cloud"></i></div>
          <h3>Cloud Architecture</h3>
          <p>AWS, Azure & GCP migration with zero data loss.</p>
          <a class="card-link" href="{{ route('contact') }}">Know More <i class="fa-solid fa-arrow-right"></i></a>
        </div>

        <div class="service-card-ref">
          <div class="service-icon-circle icon-rose"><i class="fa-solid fa-shield-halved"></i></div>
          <h3>Cybersecurity</h3>
          <p>End-to-end security audits & penetration testing.</p>
          <a class="card-link" href="{{ route('contact') }}">Know More <i class="fa-solid fa-arrow-right"></i></a>
        </div>

        <div class="service-card-ref">
          <div class="service-icon-circle icon-purple"><i class="fa-solid fa-database"></i></div>
          <h3>ERP & CRM Systems</h3>
          <p>Unified platforms for inventory, sales & HR.</p>
          <a class="card-link" href="{{ route('contact') }}">Know More <i class="fa-solid fa-arrow-right"></i></a>
        </div>

        <div class="service-card-ref">
          <div class="service-icon-circle icon-mint"><i class="fa-solid fa-brain"></i></div>
          <h3>AI & Machine Learning</h3>
          <p>Predictive analytics & intelligent workflow automation.</p>
          <a class="card-link" href="{{ route('contact') }}">Know More <i class="fa-solid fa-arrow-right"></i></a>
        </div>

        <div class="service-card-ref">
          <div class="service-icon-circle icon-sky"><i class="fa-solid fa-server"></i></div>
          <h3>DevOps & Automation</h3>
          <p>Kubernetes, CI/CD pipelines & Infrastructure as Code.</p>
          <a class="card-link" href="{{ route('contact') }}">Know More <i class="fa-solid fa-arrow-right"></i></a>
        </div>

        <div class="service-card-ref">
          <div class="service-icon-circle icon-orange"><i class="fa-solid fa-headset"></i></div>
          <h3>Managed IT Support</h3>
          <p>24/7 system monitoring & enterprise IT helpdesk.</p>
          <a class="card-link" href="{{ route('contact') }}">Know More <i class="fa-solid fa-arrow-right"></i></a>
        </div>
      </div>
    </div>
  </section>

  <!-- Advanced IT Capabilities Showcase Cards -->
  <section class="section">
    <div class="container">
      <div class="section-head">
        <div class="pill-badge">ADVANCED CAPABILITIES</div>
        <h2>Solving High-Scale Technical Challenges</h2>
        <p>We build mission-critical enterprise systems designed for reliability and zero downtime.</p>
      </div>

      <div class="portfolio-grid">
        <div class="project-card" style="padding: 34px 26px;">
          <div class="service-icon-circle icon-blue" style="margin-bottom: 20px;">
            <i class="fa-solid fa-server"></i>
          </div>
          <h3 style="font-size: 19px; font-weight: 800; color: #0f172a; margin-bottom: 10px;">Cloud Migration & Optimization</h3>
          <p style="font-size: 14px; color: var(--text-muted); line-height: 1.6; margin-bottom: 18px;">
            Seamlessly modernize legacy monoliths to containerized microservices on Kubernetes, reducing cloud hosting overheads by up to 40%.
          </p>
          <a class="card-link" href="{{ route('contact') }}">Explore Cloud Migration <i class="fa-solid fa-arrow-right"></i></a>
        </div>

        <div class="project-card" style="padding: 34px 26px;">
          <div class="service-icon-circle icon-rose" style="margin-bottom: 20px;">
            <i class="fa-solid fa-user-shield"></i>
          </div>
          <h3 style="font-size: 19px; font-weight: 800; color: #0f172a; margin-bottom: 10px;">Enterprise Security & Compliance</h3>
          <p style="font-size: 14px; color: var(--text-muted); line-height: 1.6; margin-bottom: 18px;">
            Protect your sensitive data with zero-trust architecture, SOC2/ISO compliance hardening, encrypted data vaults, and real-time vulnerability detection.
          </p>
          <a class="card-link" href="{{ route('contact') }}">Explore Cybersecurity <i class="fa-solid fa-arrow-right"></i></a>
        </div>

        <div class="project-card" style="padding: 34px 26px;">
          <div class="service-icon-circle icon-purple" style="margin-bottom: 20px;">
            <i class="fa-solid fa-gears"></i>
          </div>
          <h3 style="font-size: 19px; font-weight: 800; color: #0f172a; margin-bottom: 10px;">Custom Enterprise Software</h3>
          <p style="font-size: 14px; color: var(--text-muted); line-height: 1.6; margin-bottom: 18px;">
            Bespoke ERP and CRM software solutions tailored to your unique internal workflows, supply chain management, and automated financial audit reports.
          </p>
          <a class="card-link" href="{{ route('contact') }}">Explore Custom ERP <i class="fa-solid fa-arrow-right"></i></a>
        </div>
      </div>
    </div>
  </section>

  <!-- Work Process Section -->
  <section class="section section-alt">
    <div class="container">
      <div class="section-head">
        <div class="pill-badge">IMPLEMENTATION PROCESS</div>
        <h2>How We Deliver Enterprise Solutions</h2>
        <p>A structured 4-step engineering roadmap to transform your IT ecosystem.</p>
      </div>

      <div class="process-grid">
        <div class="process-card">
          <div class="step-num-badge icon-blue">01</div>
          <h3>Audit & Strategy</h3>
          <p>Analyzing legacy systems & security gaps.</p>
        </div>

        <div class="process-card">
          <div class="step-num-badge icon-mint">02</div>
          <h3>Architecture Design</h3>
          <p>Blueprint for cloud & scalable microservices.</p>
        </div>

        <div class="process-card">
          <div class="step-num-badge icon-purple">03</div>
          <h3>Implementation</h3>
          <p>Zero-downtime deployment & data migration.</p>
        </div>

        <div class="process-card">
          <div class="step-num-badge icon-rose">04</div>
          <h3>24/7 Management</h3>
          <p>Continuous monitoring & proactive support.</p>
        </div>
      </div>
    </div>
  </section>

  <!-- Call to Action Banner -->
  <section class="section" style="padding-top: 0;">
    <div class="container">
      <div class="cta-banner">
        <div>
          <div class="pill-badge" style="background: rgba(255,255,255,0.15); color: #fff; border-color: rgba(255,255,255,0.3);">READY TO GROW?</div>
          <h2>Modernize Your Enterprise IT Today</h2>
          <p>Speak with our senior solutions architects to get a tailored proposal.</p>
          
          <div style="display: flex; gap: 14px; flex-wrap: wrap;">
            <a class="btn btn-primary" href="{{ route('contact') }}">Get Free Consultation <i class="fa-solid fa-arrow-right"></i></a>
            <a class="btn btn-outline" href="{{ route('contact') }}">Contact Us</a>
          </div>
        </div>

        <div class="cta-img-box">
          <img src="{{ asset('assets/cta_rocket_3d.png') }}" alt="Webvire Rocket Launch 3D Graphic">
        </div>
      </div>
    </div>
  </section>
@endsection

@push('scripts')
  <script>
    document.addEventListener('DOMContentLoaded', () => {
      WebvireUI.init('it-solutions');
    });
  </script>
@endpush
